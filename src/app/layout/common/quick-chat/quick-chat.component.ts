...
export class QuickChatComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    user: import('c:/MraihiAmine/7_Freelence/34_Amal_MEAN/downloads/evalup-docs/fuse-demo-v18.0.0/src/app/core/user/user.types').User;
    ...
    createMessage() {
        ...
        this._quickChatService
            .createMessage(this.user.id, this.chat.contactId, messageText)
            .pipe(
                switchMap(() =>
                    this._quickChatService.getChatsTmp(this.user.id)
                ),
