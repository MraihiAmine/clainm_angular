...
export class InventoryService {
    ...
    private _vendors: BehaviorSubject<InventoryVendor[] | null> =
        new BehaviorSubject(null);
    private _pmos: BehaviorSubject<PMO[] | null> = new BehaviorSubject(null);
    private _programs: BehaviorSubject<ProgramResponse[] | null> =
        new BehaviorSubject(null);
    ...
    get pmosArray$(): Observable<PMO[]> {
        return this._pmos.asObservable();
    }
    ...
    get programs$(): Observable<ProgramResponse[]> {
    ...
    deleteProgram(programId: string): Observable<void> {
        return this._httpClient
            .delete(`http://localhost:3000/programs/delete/${programId}`, {
                responseType: 'text',
            })
            .pipe(
                take(1),
                map((response) => {
                    const programs = this._programs.value; // Get the current programs
                    const index = programs.findIndex(
                        (program) => program.id === programId
                    );

                    if (index !== -1) {
                        programs.splice(index, 1); // Remove the deleted program from the array
                        this._programs.next([...programs]); // Update the programs array
                    }
                })
            );
    }
    ...
    updateProgram(
        programId: string,
        updatedProgram: ProgramResponse
    ): Observable<ProgramResponse> {
        const updatedProgramId = updatedProgram.id; // Extract the ID from the updated program

        // Ensure that programs$ emits the array of programs directly
        return this.programs$.pipe(
            take(1),
            map((programs) => {
                console.log('updated in map', updatedProgram);
                const index = programs.findIndex(
                    (item) => item.id === updatedProgramId
                );
                ...
                if (index !== -1) {
                    ...
                    this._programs.next([...programs]); // Use spread operator to create a new array to trigger change detection
                }
    ...
    createProgram(): Observable<ProgramResponse> {
        const currentDate = new Date().toISOString();
        // Define default values for the program
        const defaultProgram: ProgramResponse = {
            label: 'Default Label',
            ...
            switchMap((programs) =>
                this._httpClient
                    .post<ProgramResponse>(
                        'http://localhost:3000/programs/add',
                        defaultProgram
                    )
                    .pipe(
                        map((addedProgram) => {
                            this._programs.next([addedProgram, ...programs]); // Update the programs array with the new program at the beginning
                            return addedProgram;
                        })
                    )
            )
    ...
    getProgramById(programId): Observable<ProgramResponse> {
        return this._httpClient
            .get<ProgramResponse>(
                `http://localhost:3000/programs/findById/${programId}`
            )
            .pipe(
                tap((program) => {
                    console.log('Program was retrieved', program);
    ...
    getPrograms(): Observable<ProgramResponse[]> {
        return this._httpClient
            .get<ProgramResponse[]>('http://localhost:3000/programs')
            .pipe(
                tap((programs) => {
                    console.log('Programs was called');
